...
import { ChevronDown, X, Lock, Stethoscope } from "lucide-react";
...
const DoctorAuthPage = () => {
  ...
  const handleSelect = (id: string) => {
    ...
    setError("");
    setPin(["", "", "", "", "", ""]);
    setTimeout(() => document.getElementById("dpin-0")?.focus(), 50);
  };
  ...
  return (
    <div className="min-h-screen bg-[#f0f0f8] dark:bg-[#0a0a0f] flex items-center justify-center p-6">
      ...
      <motion.div initial={{ opacity: 0, y: 20, scale: 0.97 }} animate={{ opacity: 1, y: 0, scale: 1 }}
        ...
        {/* Header */}
        <div className="mb-7">
          <p className="text-[10px] font-semibold tracking-widest text-gray-400 dark:text-gray-600 mb-2">DOCTOR SIGN-IN</p>
              ...
              <button type="button" onClick={() => setDropdownOpen(!dropdownOpen)}
                ...
                <div className="w-8 h-8 rounded-full bg-gray-100 dark:bg-gray-700 flex items-center justify-center shrink-0 overflow-hidden">
                  {selected?.photoUrl
                    ...
                    : <Stethoscope className="w-4 h-4 text-gray-500 dark:text-gray-400" strokeWidth={1.5} />}
                </div>
                  ...
                  {loadingList
                    ...
                    : selected
                      ? <>
                          ...
                          {selected.specialization && <p className="text-xs text-gray-400">{selected.specialization}</p>}
                        </>
              ...
              <AnimatePresence>
                {dropdownOpen && (
                  <motion.div initial={{ opacity: 0, y: -6 }} animate={{ opacity: 1, y: 0 }}
                    ...
                    {doctors.length === 0
                      ? <div className="px-4 py-6 text-center text-sm text-gray-400">No doctors added. Ask admin to add you.</div>
                      : doctors.map(d => (
                        <button key={d.id} type="button" onClick={() => handleSelect(d.id)}
                          className={`w-full flex items-center gap-3 px-3 py-2.5 hover:bg-gray-50 dark:hover:bg-gray-700 transition-colors text-left ${
                            selectedId === d.id ? "bg-gray-50 dark:bg-gray-700" : ""
                          }`}>
                          ...
                          <div className="flex-1 min-w-0">
                            ...
                            {d.specialization && <p className="text-xs text-gray-400 truncate">{d.specialization}</p>}
                          </div>
                        </button>
                      ))
                    }
              ...
              <motion.div initial={{ opacity: 0, y: 8 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0 }}>
                ...
                <div className="flex items-center gap-2 bg-gray-50 dark:bg-gray-800 border border-gray-200 dark:border-gray-700 rounded-xl px-4 py-3">
                  ...
                  <div className="flex gap-2 flex-1 justify-center" onPaste={handlePaste}>
                    {pin.map((digit, i) => (
                      <input key={i} id={`dpin-${i}`} type="password" autoComplete="off"
