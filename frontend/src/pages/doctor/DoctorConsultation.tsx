...
const StepHeader = ({ num, icon: Icon, label }: { num: number; icon: typeof Activity; label: string }) => (
  <div className="flex items-center gap-3 mb-4">
    <span className="text-[10px] font-semibold tracking-widest text-gray-400 dark:text-gray-600">STEP {num}</span>
    <Icon className="w-4 h-4 text-gray-400 dark:text-gray-600" strokeWidth={1.5} />
    <h3 className="text-xl font-serif text-gray-900 dark:text-white">{label}</h3>
  </div>
);
      ...
      <div className="flex flex-wrap gap-2">
        {areas.map(area => (
          <button key={area} onClick={() => { onSelect(`${complaint} - ${area}`); onClose(); }}
            className="px-3 py-1.5 rounded-full bg-gray-900 dark:bg-white text-white dark:text-gray-900 text-xs font-medium hover:opacity-90">
            {area}
          </button>
        ))}
      </div>
...
// ── Medicine Card (right panel) ───────────────────────────────────────────────
const MedCard = ({ med, onRemove, onUpdate }: {
    ...
    <div className="bg-gray-50 dark:bg-white/5 rounded-xl border border-gray-200 dark:border-white/10 overflow-hidden">
      <div className="flex items-center gap-2.5 px-3 py-2.5">
        ...
        <button onClick={() => setExpanded(!expanded)}
          className="w-7 h-7 rounded-lg flex items-center justify-center text-gray-400 hover:bg-gray-200 dark:hover:bg-white/10">
          {expanded ? <ChevronUp className="w-3.5 h-3.5" /> : <ChevronDown className="w-3.5 h-3.5" />}
        </button>
        <button onClick={() => onRemove(med.medicineId)}
          className="w-7 h-7 rounded-lg flex items-center justify-center text-gray-400 hover:text-red-500 hover:bg-red-50 dark:hover:bg-red-500/10">
...
const DoctorConsultation = () => {
  ...
  const handleComplaintClick = (c: string) => {
    const areas = COMPLAINTS_WITH_SUB[c];
    const isSel = selectedComplaints.some(x => x === c || x.startsWith(`${c} –`) || x.startsWith(`${c} -`));
    if (isSel) {
      setSelectedComplaints(prev => prev.filter(x => x !== c && !x.startsWith(`${c} –`) && !x.startsWith(`${c} -`)));
    } else if (areas) {
      setSubModal(c);
    } else {
      setPendingLabel(c);
        ...
        <div className="flex-1 overflow-y-auto">
          ...
          <div className="sticky top-0 z-10 bg-[#f5f5fa] dark:bg-[#0a0a0f] border-b border-gray-200 dark:border-white/5 px-8 py-3.5 flex items-center gap-3">
            ...
            <button onClick={() => { refreshQueue(); refreshMedicines(); }}
              className="ml-auto w-8 h-8 rounded-lg flex items-center justify-center text-gray-400 hover:text-gray-700 dark:hover:text-gray-300 hover:bg-gray-100 dark:hover:bg-white/10 transition-colors">
            ...
            <section>
              ...
              <div className="bg-white dark:bg-[#0d0d1a] border border-gray-200 dark:border-white/5 rounded-2xl p-5">
                <div className="relative mb-4">
                  <Search className="absolute left-3 top-1/2 -translate-y-1/2 w-4 h-4 text-gray-400" />
                  <input placeholder="Search or add a diagnosis…" value={diagSearch}
                        ...
                        <div className="flex-1 min-w-[180px]">
                          ...
                          <div className="flex gap-1 items-center">
                            <input type="number" placeholder="120" value={vitals.bpSys ?? ""}
                              onChange={e => updateVital("bpSys", e.target.value ? Number(e.target.value) : undefined)}
                              className="w-full h-9 rounded-xl border border-gray-200 dark:border-gray-700 bg-transparent px-3 text-sm text-gray-900 dark:text-white focus:outline-none focus:border-gray-400" />
                            <span className="text-gray-400 text-sm shrink-0">/</span>
                            <input type="number" placeholder="80" value={vitals.bpDia ?? ""}
                              onChange={e => updateVital("bpDia", e.target.value ? Number(e.target.value) : undefined)}
                              className="w-full h-9 rounded-xl border border-gray-200 dark:border-gray-700 bg-transparent px-3 text-sm text-gray-900 dark:text-white focus:outline-none focus:border-gray-400" />
        ...
        <div className="w-[340px] shrink-0 flex flex-col h-screen border-l border-gray-200 dark:border-white/5 bg-white dark:bg-[#0d0d1a] overflow-hidden">
          <div className="flex-1 overflow-y-auto px-5 py-5 space-y-5">
            ...
            <div>
              ...
              <textarea placeholder="Observations, follow-up advice…" value={doctorNotes}
                onChange={e => setDoctorNotes(e.target.value)} rows={3}
            ...
            <div>
              ...
              {followUpDate && !followUpOverride && (
                <p className="text-[10px] text-gray-400 mt-1">Auto-suggested from longest medicine duration.</p>
              )}
            </div>
          </div>
