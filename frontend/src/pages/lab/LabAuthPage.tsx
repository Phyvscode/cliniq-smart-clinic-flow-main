...
import { ChevronDown, X, Lock, FlaskConical } from "lucide-react";
...
const LabAuthPage = () => {
  ...
  const [labStaff,     setLabStaff]     = useState<StaffMember[]>([]);
  ...
  useEffect(() => {
    fetch(`${BASE_URL}/auth/staff-list?role=lab_staff`)
      .then(r => r.json())
      .then(d => setLabStaff(d.staff || []))
      .catch(() => setLabStaff([]))
      .finally(() => setLoadingList(false));
  }, []);

  const selected = labStaff.find(p => p.id === selectedId);
  ...
  const handleSelect = (id: string) => {
    ...
    setError("");
    setPin(["", "", "", "", "", ""]);
    setTimeout(() => document.getElementById("labpin-0")?.focus(), 50);
  };
  ...
  const handlePinChange = (index: number, value: string) => {
    ...
    setPin(newPin);
    if (value && index < 5) document.getElementById(`labpin-${index + 1}`)?.focus();
  };

  const handleKeyDown = (index: number, e: React.KeyboardEvent) => {
    if (e.key === "Backspace" && !pin[index] && index > 0)
      document.getElementById(`labpin-${index - 1}`)?.focus();
  };
  ...
  const handlePaste = (e: React.ClipboardEvent) => {
    ...
    document.getElementById(`labpin-${next === -1 ? 5 : next}`)?.focus();
  };
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      setPin(["", "", "", "", "", ""]);
      document.getElementById("labpin-0")?.focus();
  ...
  return (
    <div className="min-h-screen bg-[#f0f0f8] dark:bg-[#0a0a0f] flex items-center justify-center p-6">
      ...
      <motion.div initial={{ opacity: 0, y: 20, scale: 0.97 }} animate={{ opacity: 1, y: 0, scale: 1 }}
        ...
        <div className="mb-7">
          <p className="text-[10px] font-semibold tracking-widest text-gray-400 dark:text-gray-600 mb-2">LABORATORY SIGN-IN</p>
            ...
            <div className="relative" ref={dropdownRef}>
              <button type="button" onClick={() => setDropdownOpen(!dropdownOpen)}
                ...
                <div className="w-8 h-8 rounded-full bg-gray-100 dark:bg-gray-700 flex items-center justify-center shrink-0">
                  <FlaskConical className="w-4 h-4 text-gray-500 dark:text-gray-400" strokeWidth={1.5} />
                </div>
              ...
              <AnimatePresence>
                {dropdownOpen && (
                  <motion.div initial={{ opacity: 0, y: -6 }} animate={{ opacity: 1, y: 0 }}
                    ...
                    {labStaff.length === 0
                      ? <div className="px-4 py-6 text-center text-sm text-gray-400">No lab staff added. Ask admin to add you.</div>
                      : labStaff.map(p => (
              ...
              <motion.div initial={{ opacity: 0, y: 8 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0 }}>
                ...
                <div className="flex items-center gap-2 bg-gray-50 dark:bg-gray-800 border border-gray-200 dark:border-gray-700 rounded-xl px-4 py-3">
                  ...
                  <div className="flex gap-2 flex-1 justify-center" onPaste={handlePaste}>
                    {pin.map((digit, i) => (
                      <input key={i} id={`labpin-${i}`} type="password" autoComplete="off"
