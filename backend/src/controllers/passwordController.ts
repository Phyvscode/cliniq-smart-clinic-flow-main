...
export const sendOtp = asyncHandler(async (req: Request, res: Response) => {
  ...
  if ((emailAttempts?.attempts ?? 0) >= MAX_ATTEMPTS_PER_HR) {
    res.status(429).json({ message: "Too many reset requests. Please wait an hour and try again." });
    return;
  }
  if ((ipAttempts?.attempts ?? 0) >= MAX_ATTEMPTS_PER_HR * 3) {
    res.status(429).json({ message: "Too many requests from this device. Please try later." });
    return;
  }
  ...
  if (user) {
    ...
    try {
      ...
      res.status(500).json({ message: "Could not send email. Check your SMTP configuration." });
      return;
    }
  }
...
export const verifyOtp = asyncHandler(async (req: Request, res: Response) => {
  ...
  if (!record) { res.status(400).json({ message: "Invalid or expired code. Please request a new one." }); return; }
...
export const resetPassword = asyncHandler(async (req: Request, res: Response) => {
  ...
  if (!record || record.email !== normalizedEmail || record.used || record.expiresAt < now) {
    res.status(400).json({ message: "Invalid or expired session. Please request a new code." }); return;
  }
